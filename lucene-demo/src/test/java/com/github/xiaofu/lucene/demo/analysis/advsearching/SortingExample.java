...
public class SortingExample {
	 ...
	 * call the overloaded search method that accepts the custom Sort (#3). We use
	 * the useful toString method (#4) of the Sort class to describe itself, and
	 * then create PrintStream that accepts UTF-8 encoded output (#5), and finally
	 * use StringUtils (#6) from Apache Commons Lang for nice columnar output
	 * formatting. Later youùll see a reason to look at the explanation of score .
	 * For now, itùs commented out (#7).
	 */

	public static void main(String[] args) throws Exception {
		Query allBooks = new MatchAllDocsQuery();

		QueryParser parser = new QueryParser( // #1
				"contents", // #1
				new ClassicAnalyzer()); // #1
		BooleanQuery query = new BooleanQuery.Builder().add(allBooks, BooleanClause.Occur.SHOULD) // #1
				.add(parser.parse("java OR action"), BooleanClause.Occur.SHOULD).build(); // #1
		...
		example.displayResults(query, new Sort(new SortField("category", SortField.Type.STRING), SortField.FIELD_SCORE,
				new SortField("pubmonth", SortField.Type.INT, true)));

		example.displayResults(query,
				new Sort(new SortField[] { SortField.FIELD_SCORE, new SortField("category", SortField.Type.STRING) }));
		directory.close();
